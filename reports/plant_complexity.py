"""
reports/plant_complexity.py

PRODUCTION plant-complexity module: an intrinsic complexity rate (0..1) per
plant, used by reports/make_report.py. Self-contained and runnable:

    python reports/plant_complexity.py     # prints the complexity table

The methodology was prototyped in tests/probe_complexity.py (a scratch/practice
script); this file is the clean, stable version the report depends on. It is
kept here rather than imported from tests/ so the reporting tool has no
dependency on the test package.

THREE OPEN-LOOP PROBES
----------------------
  ...
  3) SWING  ("does it oscillate when nudged at the target?")
     ...
     control action. An oscillatory (underdamped) plant swings back and forth;
     a well-damped plant just slides to a new value. We count direction
     reversals (turning points) of the output:
         score = mean_target (# velocity sign changes over the horizon).
         0 -> monotonic (easy) ;  more -> oscillatory (harder).
...
same forward-Euler step the engine uses, which also bypasses control saturation
while probing.
"""
...
PROBE_NAMES = ["target_hold", "nonlinearity", "swing"]


# --- engine-direct open-loop simulation ------------------------------------
def _make_system(plant: str):
    """Build a plant and pin the probe's dt / horizon."""
...
def compute_plant_complexity(plants=None) -> dict:
    ...
    complexity = {p: sum(WEIGHTS[k] * norm[k][p] for k in PROBE_NAMES) for p in plants}
...
def print_plant_complexity(result: dict) -> None:
    """Print the raw + normalized + blended complexity table (also used by
    make_report.py so the numbers are echoed on every report run)."""
    raw, norm = result["raw"], result["normalized"]
    cx = result["complexity"]
    plants = result["plants"]
    states, actions = result["num_states"], result["num_actions"]

    print(f"\nPlant complexity  (dt={PROBE_DT}, horizon={PROBE_MAX_TIME}s, "
    ...
    print("-" * 61)
    for p in sorted(plants, key=lambda k: cx[k]):
        print(f"{p:<20}{norm['target_hold'][p]:>9.2f}{norm['nonlinearity'][p]:>9.2f}"
              f"{norm['swing'][p]:>9.2f}{cx[p]:>14.3f}")
...
def main() -> None:
    print_plant_complexity(compute_plant_complexity(PLANTS))
