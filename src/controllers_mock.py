...
def initialize_state(
    ...
    **kwargs
    ...
    # === SYSTEM & SIMULATOR CREATION (case_studies + CustomDynamicalSystem) ===
    if custom_dynamics_path:
        dyn_path = Path(custom_dynamics_path)
        if not dyn_path.exists():
            raise FileNotFoundError(f"Custom dynamics file not found: {custom_dynamics_path}")

        def factory(scenario=None, _p=str(dyn_path), _ni=num_inputs):
            return CustomDynamicalSystem(_p, scenario, num_inputs=_ni)

        simulator = SimulationRunner(factory)
    else:
        # Built-in names resolve via create_system -> case_studies/*.py
        def factory(scenario=None, _name=system_name, _ni=num_inputs):
            return create_system(_name, scenario=scenario, num_inputs=_ni)
